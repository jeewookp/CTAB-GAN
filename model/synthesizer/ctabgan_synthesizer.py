...
class Discriminator_transformer(Module):
    def __init__(self):
        ...
        encoder_layer = nn.TransformerEncoderLayer(d_model=136, nhead=8)
...
class CTABGANSynthesizer:
    ...
    def fit(self, data_prep, train_data=pd.DataFrame, eval_data=pd.DataFrame, categorical=[], mixed={}, type={}):
        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            print(syn.bad.values)
            print(syn.bad.values.dtype)
            fake_train_label = torch.from_numpy(syn.bad.values).to(self.device)
